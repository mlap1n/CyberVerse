...
class Lift(composer.Task):
  ...
  def get_reward(self, physics, old_action, action):
    hand_pos = physics.bind(self._hand.tool_center_point).xpos
    target_pos = physics.bind(self._prop.geom).xpos
    distance = np.linalg.norm(hand_pos - target_pos)
    r1 = rewards.tolerance(distance,
                          bounds=(0, 0.05),
                          margin=0.0,
                          sigmoid="long_tail")

    prop_height = self._get_height_of_lowest_vertex(physics)
    r2 = rewards.tolerance(prop_height,
